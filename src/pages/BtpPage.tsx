...
import { ArrowRight, Cloud, Server, Zap, Shield, Settings, Database, BarChart3, TrendingUp, PieChart, Activity, Brain, ChevronRight, Target, Layers, RefreshCw, FileCheck, Gauge, HeartPulse, Search, Compass, Code2, FlaskConical, Rocket, HeartHandshake, Play, Pause, Workflow, GitBranch, Globe, Lock, Cpu, Bot, Puzzle, Cable, AppWindow, Blocks, DollarSign } from "lucide-react";
    ...
    <section className="relative min-h-[92vh] flex items-center overflow-hidden" style={{ background: "#0B0B0B" }}>
      ...
      <div className="relative z-10 max-w-[1320px] mx-auto px-6 md:px-10 grid lg:grid-cols-2 gap-16 items-center py-24">
        <motion.div initial="hidden" animate="visible" variants={stagger}>
          <motion.span variants={fadeUp} className="inline-block text-[.7rem] font-bold tracking-[.25em] uppercase mb-5" style={{ color: "#F4B400" }}>SAP BTP Expertise</motion.span>
          <motion.h1 variants={fadeUp} className="text-[clamp(2.2rem,5vw,3.8rem)] font-bold leading-[1.08] tracking-tight text-white mb-6">
            Accelerate Innovation with <span style={{ color: "#F4B400" }}>SAP BTP</span>
          </motion.h1>
          <motion.p variants={fadeUp} className="text-[1.05rem] leading-relaxed mb-8" style={{ color: "#C9C9C9" }}>
            SAP Business Technology Platform is the unified platform for data management, analytics, application development, integration, and AI. Kannanware helps enterprises unlock the full potential of BTP to extend, integrate, and innovate across your SAP landscape.
          </motion.p>
          <motion.div variants={fadeUp} className="flex flex-wrap gap-4">
            ...
            <a href="#demo" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg font-semibold text-sm text-white border transition-all hover:bg-white/5" style={{ borderColor: "rgba(244,180,0,.35)" }}>Explore BTP Demo</a>
          </motion.div>
        </motion.div>
        {/* Right — animated BTP dashboard */}
        <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: .3 }} className="relative hidden lg:block">
          <div className="relative rounded-2xl overflow-hidden border p-6" style={{ background: "rgba(255,255,255,.03)", borderColor: "rgba(244,180,0,.12)" }}>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[{ label: "Extensions", val: "142", icon: Puzzle, delta: "+18 this month" }, { label: "Integrations", val: "89", icon: Cable, delta: "Active flows" }, { label: "API Calls", val: "2.4M", icon: Globe, delta: "+34% ↑" }].map((k, i) => (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }} className="rounded-xl p-5 border mb-3" style={{ background: "rgba(255,255,255,.02)", borderColor: "rgba(244,180,0,.06)" }}>
              <div className="flex justify-between items-center mb-4"><span className="text-xs text-white/50">Platform Utilization — Last 12 Months</span><span className="text-[.65rem] px-2 py-0.5 rounded" style={{ background: "rgba(244,180,0,.15)", color: "#F4B400" }}>Live</span></div>
              <div className="flex items-end gap-1.5 h-28">
                {[30, 40, 45, 55, 50, 65, 70, 78, 82, 88, 92, 100].map((h, i) => (
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.8 }} className="flex items-start gap-3 rounded-lg p-3 border" style={{ background: "rgba(244,180,0,.05)", borderColor: "rgba(244,180,0,.12)" }}>
              ...
              <p className="text-[.75rem] leading-relaxed text-white/70"><span className="font-semibold text-white">AI Insight:</span> Extension adoption rate increased 42% after BTP migration. Recommend scaling integration suite capacity.</p>
            </motion.div>
          </div>
...
/* ─── Section 2: BTP Pillars Toggle ─── */
function PillarsToggle() {
  ...
  const tabs = [
    {
      ...
      features: [
        { icon: AppWindow, title: "SAP Build Apps", desc: "No-code/low-code application development for citizen developers — build enterprise-grade apps without writing a single line of code." },
        { icon: Code2, title: "SAP Build Code", desc: "AI-powered pro-code development with Joule copilot, optimized for building and extending SAP applications with Java and JavaScript." },
        { icon: Blocks, title: "Cloud Foundry & Kyma", desc: "Deploy microservices and serverless functions on managed runtimes with full Kubernetes support and enterprise-grade scalability." },
        { icon: Bot, title: "SAP Build Process Automation", desc: "Automate workflows and business processes with intelligent RPA bots and no-code process builders." },
      ]
    },
    {
      label: "Integration",
      features: [
        { icon: Cable, title: "SAP Integration Suite", desc: "Connect any application — cloud or on-premise — with pre-built connectors, API management, and event-driven architecture." },
        { icon: Workflow, title: "Process Orchestration", desc: "Design, monitor, and manage complex integration scenarios with real-time visibility and intelligent error handling." },
        { icon: Globe, title: "API Management", desc: "Publish, manage, and monetize APIs with enterprise governance, rate limiting, and developer portal capabilities." },
        { icon: Lock, title: "Identity & Access", desc: "Centralized identity management with SSO, MFA, and fine-grained authorization across your entire BTP landscape." },
      ]
    },
    {
      label: "Data & Analytics",
      features: [
        { icon: Database, title: "SAP Datasphere", desc: "Unified data layer harmonizing SAP and non-SAP data sources for trusted, real-time business insights at enterprise scale." },
        { icon: BarChart3, title: "SAP Analytics Cloud", desc: "Enterprise BI, planning, and predictive analytics in one platform — from boardroom dashboards to operational reporting." },
        { icon: Brain, title: "SAP AI Core", desc: "Train, deploy, and manage AI/ML models at scale with enterprise governance, bias detection, and explainability built in." },
        { icon: Cpu, title: "SAP HANA Cloud", desc: "In-memory cloud database with multi-model capabilities — columnar, graph, spatial, and document store in a single engine." },
      ]
    }
  ];
    ...
    <section ref={ref} className="py-28 px-6 md:px-10" style={{ background: "#111111" }}>
      <div className="max-w-[1100px] mx-auto">
        <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="text-center mb-14">
          <motion.span variants={fadeUp} className="inline-block text-[.7rem] font-bold tracking-[.25em] uppercase mb-4" style={{ color: "#F4B400" }}>Platform Pillars</motion.span>
          <motion.h2 variants={fadeUp} className="text-[clamp(1.8rem,4vw,2.8rem)] font-bold text-white tracking-tight">BTP Capability Areas</motion.h2>
        </motion.div>
        {/* Tabs */}
          ...
          <div className="relative flex rounded-full p-1 gap-1" style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(244,180,0,.12)" }}>
            {tabs.map((t, i) => (
              <button key={t.label} onClick={() => setActiveTab(i)} className={`relative z-10 px-6 py-3 rounded-full text-sm font-semibold transition-colors ${activeTab === i ? "text-black" : "text-white/60 hover:text-white"}`}>
                {activeTab === i && <motion.div layoutId="btp-tab" className="absolute inset-0 rounded-full" style={{ background: "#F4B400" }} />}
    ...
    <section ref={ref} id="demo" className="py-28 px-6 md:px-10" style={{ background: "#0B0B0B" }}>
      <div className="max-w-[1100px] mx-auto">
        <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="text-center mb-14">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(1.8rem,4vw,2.8rem)] font-bold text-white tracking-tight">Live BTP Integration Monitor</motion.h2>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: .7 }} className="rounded-2xl border overflow-hidden" style={{ background: "rgba(255,255,255,.02)", borderColor: "rgba(244,180,0,.1)" }}>
          <div className="flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: "rgba(255,255,255,.06)" }}>
            ...
            <span className="text-[.65rem] text-white/30 tracking-wider uppercase">BTP Integration Console</span>
            <div />
          </div>
          <div className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[{ l: "Active Flows", v: done ? 156 : 89, s: "", u: "" }, { l: "Messages/Hr", v: done ? 48200 : 12400, s: "", u: "" }, { l: "Success Rate", v: done ? 99 : 94, s: "", u: "%" }, { l: "Avg Latency", v: done ? 23 : 85, s: "", u: "ms" }].map((k) => (
            ...
            <div className="rounded-xl p-5 border mb-5" style={{ background: "rgba(255,255,255,.015)", borderColor: "rgba(255,255,255,.05)" }}>
              <div className="flex items-end gap-2 h-36">
                {[25, 35, 30, 50, 45, 60, 55, 70, 65, 78, 72, done ? 95 : 60].map((h, i) => (
            ...
            <AnimatePresence>
              {done && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0 }} className="rounded-xl p-4 border mb-5 flex items-start gap-3" style={{ background: "rgba(244,180,0,.06)", borderColor: "rgba(244,180,0,.15)" }}>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white mb-1">AI Optimization Detected</p>
                    <p className="text-[.8rem] leading-relaxed" style={{ color: "#C9C9C9" }}>"Integration flow 'Order-to-Fulfillment' bottleneck identified. Auto-scaled processing nodes from 3→8. Latency reduced by 73%. Recommend permanent capacity increase."</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            <div className="flex justify-center">
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: .97 }} onClick={runAnalysis} disabled={running} className="inline-flex items-center gap-2 px-8 py-3.5 rounded-lg font-semibold text-sm text-black disabled:opacity-70 transition-shadow" style={{ background: "#F4B400", boxShadow: "0 0 30px rgba(244,180,0,.25)" }}>
                {running ? <><Activity size={16} className="animate-spin" /> Optimizing Flows…</> : <><Workflow size={16} /> Run Integration Analysis</>}
              </motion.button>
            </div>
          </div>
        </motion.div>
...
function ExpertiseSection() {
  ...
  const capabilities = [
    { icon: Puzzle, title: "Extension Development", desc: "Build side-by-side extensions on SAP BTP that extend S/4HANA, SuccessFactors, and Ariba without modifying the core — keeping your clean core strategy intact while delivering business-specific capabilities.", num: "01" },
    { icon: Cable, title: "Enterprise Integration", desc: "Design and implement integration architectures using SAP Integration Suite — connecting cloud, on-premise, and third-party applications with pre-built adapters, API management, and event mesh.", num: "02" },
    { icon: Brain, title: "AI & Machine Learning", desc: "Leverage SAP AI Core and AI Launchpad to train, deploy, and operationalize ML models. From intelligent document processing to predictive maintenance — AI at enterprise scale.", num: "03" },
    { icon: Database, title: "Data Architecture", desc: "Design unified data landscapes with SAP Datasphere and HANA Cloud. Federated data access, real-time replication, and governed data products for trusted analytics across your organization.", num: "04" },
    { icon: Workflow, title: "Process Automation", desc: "Automate end-to-end business processes with SAP Build Process Automation. Intelligent RPA bots, workflow management, and decision automation — reducing manual effort by up to 80%.", num: "05" },
    { icon: Lock, title: "Security & Governance", desc: "Implement enterprise-grade security with SAP Cloud Identity Services, trust configurations, and compliance frameworks. Role-based access, audit logging, and data privacy at every layer.", num: "06" },
  ];
    ...
    <section ref={ref} className="py-28 px-6 md:px-10" style={{ background: "#111111" }}>
      <div className="max-w-[1200px] mx-auto">
        <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="text-center mb-20">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(1.8rem,4vw,2.8rem)] font-bold text-white tracking-tight">Our BTP Implementation Expertise</motion.h2>
        </motion.div>
...
function TransformationTimeline() {
  ...
  const steps = [
    { title: "Platform Assessment", icon: Search, desc: "Evaluate your current landscape, identify extension and integration opportunities, and define your BTP adoption roadmap. We assess workloads, licenses, and technical readiness.", img: teamDiscovery },
    { title: "Architecture Design", icon: Compass, desc: "Design the target BTP architecture — subaccount structure, service selection, security model, and CI/CD pipelines. Our certified architects plan for multi-region, multi-environment deployments.", img: teamDesign },
    { title: "Build & Integrate", icon: Code2, desc: "Develop extensions, configure integration flows, and build custom applications using SAP Build and Cloud Foundry. Agile sprints with continuous deployment and testing.", img: teamImplement },
    { title: "Quality Assurance", icon: FlaskConical, desc: "Comprehensive testing of all BTP services — API testing, integration flow validation, performance benchmarks, and security penetration testing across the platform.", img: teamTesting },
    { title: "Go-Live & Migration", icon: Rocket, desc: "Production deployment with blue-green strategies, traffic routing, and real-time monitoring. Seamless cutover from legacy middleware to BTP Integration Suite.", img: teamGolive },
    { title: "Managed Services", icon: HeartHandshake, desc: "Ongoing platform management, monitoring, and optimization. Proactive incident management, cost optimization, and continuous innovation advisory for your BTP landscape.", img: teamHypercare },
  ];
    ...
    <section ref={ref} className="py-28 px-6 md:px-10 overflow-hidden" style={{ background: "#0B0B0B" }}>
      <div className="max-w-[1200px] mx-auto">
        <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(1.8rem,4vw,2.8rem)] font-bold text-white tracking-tight mb-3">BTP Adoption Journey</motion.h2>
...
function ArchitectureViz() {
  ...
  const nodes = [
    { label: "SAP Build", desc: "Low-code/pro-code development environment for building enterprise applications, process automations, and business sites — all with AI-assisted development via Joule.", color: "#F4B400" },
    { label: "Integration Suite", desc: "Enterprise iPaaS connecting 3,000+ applications with pre-built adapters. API management, event mesh, and trading partner management in one platform.", color: "#FFD54F" },
    { label: "HANA Cloud", desc: "Multi-model cloud database engine — columnar, graph, spatial, and document store. In-memory processing with native data tiering to data lake.", color: "#FFA726" },
    { label: "AI Core", desc: "Enterprise AI infrastructure for training and serving ML models. Generative AI Hub integration with foundational models from SAP, OpenAI, Google, and Meta.", color: "#FFB74D" },
    { label: "Datasphere", desc: "Business data fabric unifying data across SAP and non-SAP sources. Data marketplace, transformation flows, and governed consumption layers.", color: "#FFCA28" },
    { label: "Analytics Cloud", desc: "Unified BI, planning, and predictive platform. Smart Insights, Just Ask natural language queries, and collaborative enterprise planning.", color: "#FFE082" },
  ];
    ...
    <section ref={ref} className="py-28 px-6 md:px-10" style={{ background: "#111111" }}>
      <div className="max-w-[1200px] mx-auto">
        <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="text-center mb-10">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(1.8rem,4vw,2.8rem)] font-bold text-white tracking-tight">BTP Ecosystem</motion.h2>
          ...
          <div className="relative flex justify-center" onMouseEnter={() => { pausedRef.current = true; }} onMouseLeave={() => { pausedRef.current = false; }}>
            <div className="relative" style={{ width: r * 2 + 120, height: r * 2 + 120 }}>
              ...
              <motion.div initial={{ scale: 0 }} animate={inView ? { scale: 1 } : {}} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28 rounded-full flex items-center justify-center z-10" style={{ background: "radial-gradient(circle, rgba(244,180,0,.35) 0%, rgba(244,180,0,.08) 70%)", border: "2px solid rgba(244,180,0,.5)", boxShadow: "0 0 60px rgba(244,180,0,.2), 0 0 120px rgba(244,180,0,.08)" }}>
                <span className="text-sm font-bold text-white text-center leading-tight">SAP<br/>BTP</span>
              </motion.div>
              {nodes.map((n, i) => {
            ...
            <AnimatePresence mode="wait">
              {selected !== null ? (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center w-full">
                  <p className="text-lg text-white/40">Click any orbiting node to explore SAP BTP services</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
function BusinessImpact() {
  ...
  const inView = useInView(ref, { once: true, margin: "-80px" });
  const blocks = [
    { title: "Accelerated Time-to-Market", desc: "Launch new applications and integrations in weeks instead of months with low-code development, pre-built templates, and AI-assisted coding.", metric: "5x", metricLabel: "Faster Delivery" },
    { title: "Unified Data Intelligence", desc: "Break data silos with federated data access across SAP and non-SAP systems. Real-time harmonized views for trusted decision-making.", metric: "360°", metricLabel: "Data Visibility" },
    { title: "Reduced Integration Complexity", desc: "Replace point-to-point integrations with a managed iPaaS. Pre-built connectors, event-driven architecture, and API-first design.", metric: "80%", metricLabel: "Less Complexity" },
    { title: "Future-Proof Innovation", desc: "Access the latest SAP innovations — generative AI, machine learning, and IoT — all within a governed enterprise platform.", metric: "∞", metricLabel: "Scalability" },
  ];
  ...
  return (
    <section id="cta" className="relative py-28 px-6 md:px-10 overflow-hidden" style={{ background: "#0B0B0B" }}>
      ...
      <div className="relative z-10 max-w-[800px] mx-auto text-center">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-[clamp(1.8rem,4.5vw,3rem)] font-bold text-white tracking-tight mb-6">Unlock the Power of SAP BTP</motion.h2>
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: .2 }} className="text-[1.05rem] leading-relaxed mb-10" style={{ color: "#C9C9C9" }}>Partner with Kannanware to architect, build, and scale your SAP BTP landscape — from integration modernization to AI-powered innovation.</motion.p>

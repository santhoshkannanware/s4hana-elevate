          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 relative">
            {processes.map((p, i) => (
              <motion.div
                ...
                className="bg-card border border-border rounded p-5 text-center hover:border-electric-blue/30 transition-colors duration-200 relative"
              >
                <div className="font-heading text-lg font-bold text-electric-blue mb-1">{p.icon}</div>
                ...
                {/* Connector dot */}
                <div className="hidden lg:block absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-background border-2 border-electric-blue/30" />
              </motion.div>
            ))}
          </div>
        </div>
